...
//     duration: searchParams.get("duration"), // assume it's number of months as string
//     // Benefits will be passed as a JSON stringified array, parse it here:
//     benefits: (() => {
//       try {
...
//     setMessage(null);
//     try {
//       const res = await fetch(`http://localhost:4000/api/payment/${paymentId}`, {
//         method: "PUT",
//         headers: {
...
//       },
//       body: JSON.stringify({
//         membershipPlanId: useSearchParams.get("membershipPlanId"), 
//       }),
//     });
...
export default function PaymentPage() {
  ...
  const paymentId = searchParams.get("paymentId");
  const token = searchParams.get("token");
  const membershipPlanId = searchParams.get("membershipPlanId"); // ✅ Needed for membership
  ...
  const createMembershipAfterPayment = async () => {
    try {
      const res = await fetch("http://localhost:4000/api/userMembership", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handlePayClick = async () => {
    ...
    try {
      ...
      if (data.success) {
        setMessage({ type: "success", text: "🎉 Payment status updated successfully!" });
        await createMembershipAfterPayment(); // ✅ calling membership creation
      } else {
        setMessage({ type: "error", text: data.message || "Failed to update payment status" });
      }
    } catch (err) {
